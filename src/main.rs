...
impl Game {
    ...
    fn building_action_buttons(&self) -> Option<(Rect, Rect, Rect)> {
        ...
        let h = 36.0;
        let y2 = screen_height() - 28.0 - h; // Fjern (nederst, full bredde)
            ...
            if let Some(start) = self.drag_start.take() {
                if (mouse - start).length() < 6.0 {
                    ...
                    if let Some(i) = hit_unit {
                        ...
                        // Trykk pa egen bygning -> velg den.
                        self.clear_selection();
                        self.buildings[i].selected = true;
                    } else if self.has_player_selection() {
                        // Tomt punkt med noe markert -> flytt dit. Tapp-for-a-flytte
                        // sa man kan styre uten hoyreklikk (touch/iPhone).
                        self.move_selected(w);
                    } else {
                        // Tomt punkt uten markering -> ingenting a gjore.
                        self.clear_selection();
                    }
                } else {
    ...
    fn ui_burger(&self) -> Rect {
        // ~50% storre enn for, lettere a treffe pa mobil.
        Rect::new(screen_width() - 58.0, 4.0, 52.0, 36.0)
    }
    fn ui_dev_btn(&self) -> Rect {
        Rect::new(10.0, screen_height() - 34.0, 58.0, 26.0)
    }
    fn ui_lang_btn(&self) -> Rect {
        Rect::new(74.0, screen_height() - 34.0, 52.0, 26.0)
    }
    ...
    fn point_in_ui(&self, p: Vec2) -> bool {
    ...
    fn handle_ui(&mut self) {
        ...
        if pressed {
            ...
            if self.ui_lang_btn().contains(m) {
                self.lang_open = true;
    ...
    fn draw_controls(&self) {
        ...
        // Dev- og sprakknapp (nede til venstre)
